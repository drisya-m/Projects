...
class Blockchain:
    ...
    def __process_transactions(self, transactions):
        ...
        print(f'Pending transactions before adding to block:\n{transactions}\n\n')
        result = True
        for tran in transactions:
            sender = tran['message']['sender']
            receiver = tran['message']['receiver']
            sender_balance = self._accounts[sender]._balance
            if sender_balance >= tran['message']['value']:
                self._accounts[sender].decrease_balance(tran['message']['value'])
                self._accounts[receiver].increase_balance(tran['message']['value'])
            else:
                print(f'ERROR: Transaction: {tran} failed due to insufficient balance in sender account')
    ...
    def __validate_complete_account_balances(self):
        ...
        # Return False otherwise
        account_list = self.get_account_balances()
        for account in account_list:
            bal = account['initial_balance']
            for index in range(1, len(self._chain)):
                tran_list = self._chain[index].transactions
                for item in tran_list:
                    if(item['message']['sender'] == account['id']):
                        bal -= item['message']['value']
                    elif(item['message']['receiver'] == account['id']):
                        bal += item['message']['value']

                    if bal < 0:
                        print(f"Balance for {account['id']} turned negative for {item}")
                        return False
    ...
    def get_account_balances(self):
        return [{'id': account.id, 'balance': account.balance, 'initial_balance': account.initial_balance}
