...
class Node:
    ...
    def get_data(self, key):
        ...
        new_node = self._vnode_map.get_assigned_node(key)
        if (new_node == self._name):
            data = self._data_store[key]
        else:
            val = self._node_dict[new_node]
            data = val._data_store[key]
        return data
    ...
    def set_data(self, key, value, force=False):
        if (force):
            ...
            set_node = self._node_dict[node_name]
            set_node._data_store[key] = copy.deepcopy(value)
    ...
    def add_new_node(self, new_node_name, new_node):
        ...
        for vnode in range(self._TOTAL_VIRTUAL_NODES):
            node = self._vnode_map.get_node_for_vnode(vnode)
            if (node == self._name):
        ...
        for vnode in local_vnode_slice:
            ...
            for key in self._data_store:
                temp = key % self._TOTAL_VIRTUAL_NODES
                if (temp == vnode):
                    local_key_list.append(key)
            transfer_dict[vnode] = {'target_node': new_node_name, 'keys': local_key_list}
    ...
    def remove_current_node(self, new_node_dict):
        ...
        for vnode, node in transfer_node_mapping.items():
            local_key_list = []
            for key in self._data_store:
                temp = key % self._TOTAL_VIRTUAL_NODES
                if (temp == vnode):
                    local_key_list.append(key)
            transfer_dict[vnode] = {'target_node': node, 'keys': local_key_list}
