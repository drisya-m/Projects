...
class VirtualNodeMap:
    ...
    def populate_map(self):
        ...
        for vnode in vnode_list:
            self._vnode_map[vnode] = self._node_names[vnode % len(self._node_names)]
        vnode_per_node = {}
        for i in self._vnode_map:
            value = self._vnode_map[i]
            if value in vnode_per_node:
                vnode_per_node[value] += 1
            else:
                vnode_per_node[value] = 1
        print(f'The vnode-node mapping is:\n ', self._vnode_map)
        print(f'The number of vnodes per node is:\n', vnode_per_node)
        return
